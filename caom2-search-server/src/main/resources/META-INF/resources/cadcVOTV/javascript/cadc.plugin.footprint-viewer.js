   ...
   */
     ...
     */
        function init(_viewer) {
            ...
            if (inputs.toggleSwitchSelector !== null) {
                ...
                if (
                    ...
                    $toggleSwitchSelector.html(
                        $toggleSwitchSelector.data('open')
                    );
                }
       ...
       */
            ...
            _self.aladin = A.aladin(
                inputs.targetSelector,
                inputs.aladin_options
            );
            ...
            if (_self.grid.getData().getLength) {
                if (inputs.renderedRowsOnly === true) {
                    ...
                    _self.viewer.subscribe(cadc.vot.events.onRowAdded, function(
                        e,
                        args
                    ) {
                    ...
                    _self.viewer.subscribe(
                        cadc.vot.events.onDataLoaded,
                        function() {
                            _setFieldOfView();
                        }
                    );
            ...
            if (inputs.onHover === true) {
                _self.handler.subscribe(
                    _self.grid.onMouseEnter,
                    handleMouseEnter
                );
                _self.handler.subscribe(
                    _self.grid.onMouseLeave,
                    handleMouseLeave
                );
            }
     ...
     */
        ...
        function sanitizeFootprint(nextFootprint) {
            ...
            if (nextFootprint !== null && $.trim(nextFootprint).length > 0) {
                ...
                for (
                    var fei = 0, fel = footprintElements.length;
                    fei < fel;
                    fei++
                ) {
        ...
        function handleAddFootprint(e, args) {
            ...
            if (polygonValue !== null) {
                ...
                for (var fpvi = 0; fpvi < footprintValuesLength; fpvi++) {
                    var nextFootprint = sanitizeFootprint(
                        footprintValues[fpvi]
                    );
                    ...
                    if (nextFootprint !== null) {
                        ...
                        if (!inputs.fov || inputs.fov === null) {
                            _updateFOV(
                                nextFootprint.substr(POLYGON_SPLIT.length)
                            );
                        }
                    }
                }
