;(function($, window, undefined) {
   ...
   */
  function ResultStateSerializer(
    _baseUrl,
    _sortColumn,
    _sortDirection,
    _columns,
    ...
    function getColumnParameter(_index, _columnID) {
      ...
      parameter.push(';')
      parameter.push(
        _self.widths[_columnID]
          ? encodeURIComponent(_self.widths[_columnID])
          : ''
      )
      parameter.push(';')
      parameter.push(
        _self.filters[_columnID]
          ? encodeURIComponent(_self.filters[_columnID])
          : ''
      )
      parameter.push(';')
      ...
      if (hashString) {
        $.each(hashString.split('&'), function(index, hashStringItem) {
          ...
          if (key === cadc.vot.ResultState.SORT_COLUMN) {
            ...
            var columnNumber = key.substring(
              cadc.vot.ResultState.COLUMN_PARAMETER_NAME.length
            )
