...
function DataTrain(_modelDataSource, _autoInitFlag)
{
  ...
  function load(data)
  {
    ...
    // Put the uType names into an array.
    group.uTypes = _self.uTypes;

    // Put the uType values into an array.
    group.values = [];
  ...
  function buildTable(_group)
  {
         ...
         i < groupUTypesLength; i++)
    {
      ...
      var columnOptionObj = columnManager.getColumnOption(_modelDataSource +
                                                          ":" + uType.name);
      var enumerated = {
        utype: uType.name, hidden: false,
                      ...
                      updateAllOptions)
  {
    ...
    for (var i = selectIndex, gul = group.uTypes.length; i < gul; i++)
    {
      ...
      if (select != null)
      {
        ...
        {
          // Obtain only those fresh instruments that are part of the selected
          // items.
